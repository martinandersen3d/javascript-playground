<template><div id="app">
  ...
  <!-- Menu items -->
  <span v-for="(item, index) in sidebarItems">
    <h6 v-if="item.description == ''">
      {{niceString(item.component)}}
    </h6>
    <a href="#" class="w3-bar-item w3-button link" @click="active = item.component, description = item.description  "  v-if="item.description != ''">{{ niceString(item.component)}}</a>
  </span>
  </div>
...
data(){ return{
      ...
      sidebarItems:[
        ...
        {component:"NewTemplate", description:"Aaasasasasas"},
...
.pageContent{
  margin:16px; border:1px solid #f2f2f2;
  padding:0px;
}
.json, pre {
  font-size: 12px;
